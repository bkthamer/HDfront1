...
const fetchHelicesCount = async () => {
  const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/materiels`)
  const data = await response.json()
  totalHelices.value = data.filter(
    m => m.typemateriel === 'HeliceZH3' 
  ).length
  updateChart()
}
...
<template>
  <div class="card">
    <div class="card-header">
      <h4>Total des hélices enregistrées</h4>
    </div>
    <div class="card-body">
